# Telegram Client Channel for nanobot

nanobot 的 Telegram Client API 插件，使用 [Telethon](https://github.com/LonamiWebs/Telethon) 通过 MTProto 协议以**用户账号**（非 Bot）接入 Telegram。

## 与 Bot API 的区别

| | Bot API (内置) | Client API (本插件) |
|---|---|---|
| 账号类型 | Bot 账号 | 普通用户账号 |
...
> ⚠️ 使用用户账号进行自动化可能违反 Telegram 服务条款，建议使用专用小号。
...
# session 文件保存到 ~/.nanobot/nanobot_client.session
```
...
| `sessionName` | string | `"nanobot_client"` | SQLite session 文件名（不含 .session） |
...
```
telegram-client-sikill/
├── channel/
│   ├── telegram_client.py   # TelegramClientChannel + TelegramClientConfig
...
**无侵入设计**：插件自带 `TelegramClientConfig`，通过 nanobot 的插件 channel 机制（`ChannelsConfig(extra="allow")` + `config_class`）加载，不需要修改 nanobot 的 `schema.py`。
